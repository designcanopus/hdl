...
adi_ip_create axi_ad4857

adi_ip_files axi_ad4857 [list \
    ...
    "axi_ad4857_constr.ttcl" \
    "axi_ad4857_cmos.v" \
    "axi_ad4857_16b_channel.v" \
    "axi_ad4857_20b_channel.v" \
    "axi_ad4857_crc.v" \
    "axi_ad4857_lvds.v" \
    "axi_ad4857.v" ]

adi_ip_properties axi_ad4857

# Register the ttcl constraint template to be generated as XDC
adi_ip_ttcl axi_ad4857 "axi_ad4857_constr.ttcl"
...
adi_init_bd_tcl
adi_ip_bd axi_ad4857 "bd/bd.tcl"

set_property company_url {https://wiki.analog.com/resources/fpga/docs/axi_ad4857} $cc
...
for {set i 0} {$i < 8} {incr i} {
  ...
  if { $i >= 4 } {
    set_property value_tcl_expr {expr {[info exists LVDS_CMOS_N] && [info exists DEVICE] ? $LVDS_CMOS_N == 1 ? 0 : 1 : 0}} [ipx::get_user_parameters LANE_${i}_ENABLE -of_objects $cc]
    set_property enablement_tcl_expr {expr {$LVDS_CMOS_N == 1 ? 0 : 1}} [ipx::get_user_parameters LANE_${i}_ENABLE -of_objects $cc]
  } else {
    set_property value_tcl_expr {expr {[info exists LVDS_CMOS_N] ? $LVDS_CMOS_N == 0 : 0}} [ipx::get_user_parameters LANE_${i}_ENABLE -of_objects $cc]
    set_property enablement_tcl_expr {expr $LVDS_CMOS_N == 0} [ipx::get_user_parameters LANE_${i}_ENABLE -of_objects $cc]
  }

  if { $i >= 4 } {
    adi_set_ports_dependency "lane_$i" \
      "((spirit:decode(id('MODELPARAM_VALUE.LANE_${i}_ENABLE')) == 1) and (spirit:decode(id('MODELPARAM_VALUE.LVDS_CMOS_N')) == 0))"
  } else {
    adi_set_ports_dependency "lane_$i" \
      "(spirit:decode(id('MODELPARAM_VALUE.LANE_${i}_ENABLE')) == 1) and (spirit:decode(id('MODELPARAM_VALUE.LVDS_CMOS_N')) == 0)"
  }
  ...
  if { $i >= 4 } {
    adi_set_ports_dependency "adc_enable_$i" \
      "(spirit:decode(id('MODELPARAM_VALUE.DEVICE')) == 'AD4857')"
...
set_property value AD4857 [ipx::get_user_parameters DEVICE -of_objects $cc]
set_property value AD4857 [ipx::get_hdl_parameters DEVICE -of_objects $cc]
...
set_property value_tcl_expr {expr {[info exists DEVICE] ? $DEVICE eq {AD4857} ? 15 : 31 : 31}} [ipx::get_user_parameters DW -of_objects $cc]
...
adi_set_ports_dependency "scko" \
  "(spirit:decode(id('MODELPARAM_VALUE.LVDS_CMOS_N')) == 0) and (spirit:decode(id('MODELPARAM_VALUE.ECHO_CLK_EN')) = 1)"
...
adi_set_ports_dependency "scko_p" \
  "(spirit:decode(id('MODELPARAM_VALUE.LVDS_CMOS_N')) == 1) and (spirit:decode(id('MODELPARAM_VALUE.ECHO_CLK_EN')) = 1)"
